...
from multi_meta_ssd.downstream_processors import utils_lareqa
from transformers import BertConfig, BertTokenizer, XLMRobertaTokenizer
from multi_meta_ssd.downstream_models.dual_encoders.bert import BertForRetrieval
from multi_meta_ssd.downstream_processors.utils_lareqa import mean_avg_prec_at_k_eval
...
def read_data(model, mode,  languages):
    ...
    c_features = [tokenizer.encode_plus((candidate.sentence+candidate.context).replace("\n", ""),
                                            max_length=64,
                                            pad_to_max_length=True) for candidate in candidate_set.as_list()]
...
def print_train_qry_results():
    for mode in ["mono_mono", "mono_bil", "bil_multi", "mixt"]:
        ...
        print(mode, " train_loss_qry:", np.mean([new_train_loss_qry[ep][i] for i in range(len(new_train_loss_qry[ep]))]),
         " train_precision_qry:", np.mean([train_precision_qry[1][i] for i in range(len(train_precision_qry[1]))]))
