...
def get_config_params(args):
    ...
    args.model_type = str(params.get("PARAMS", "MODEL_TYPE"))
    args.model_name_or_path = str(params.get("PARAMS", "MODEL_NAME_OR_PATH"))
...
def multilingual_zero_shot_evaluation(question_set, candidate_set, query_languages, tokenizer, base_model,  meta_learn_split_config, args, split_name):
    ...
    if use_base_model:
        ...
        all_candidate_vecs = np.concatenate([np.expand_dims(np.squeeze(candidate.encoding), 0) for candidate in candidate_set[split_name].as_list()], axis=0)
                ...
                for batch_step in tqdm(range(0, len(meta_tasks[split_name])//(n_tasks_batch*n_triplets), n_tasks_batch*n_triplets)): # number of train batches TODO change this
                    ...
                    if args.use_triplet_loss:
                        ...
                        for k in range(0, n_tasks_batch*n_triplets, n_triplets):
                            ...
                            print("concatenated_spt_features_triplets.keys():", concatenated_spt_features_triplets.keys())
                            ...
                            print("SUPPORT len_a//len_q:", len_a//len_q)
                            ...
                            print("concatenated_spt_features_triplets_all:", concatenated_spt_features_triplets_all)
                    ...
                    if args.use_meta_learn:
                        save_torch_model(args, meta_learner.base_model, meta_learner.opt, checkpoints_dir, str(ep))
                    else:
                        save_torch_model(args, base_model, opt, checkpoints_dir, str(ep))
