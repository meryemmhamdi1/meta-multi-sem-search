...
import numpy as np
import multi_meta_ssd.downstream_processors.utils_lareqa as utils_lareqa
...
def read_lareqa():
    ...
    ## Create meta-dataset tasks from LAREQA
    # print("train_dataset:", train_dataset)
    # for lang in languages:
    #     print("language:", lang, " question_set:", len(question_set.by_lang[lang]), " candidate_set:", len(candidate_set.by_lang[lang]))
    #     for i in range(10):
    #         # print(question_set.by_lang[lang][i].question, candidate_set.by_lang[lang][i].sentence, question_set.by_lang[lang][i].uid, candidate_set.by_lang[lang][i].uid)
    #         # print(question_set.by_lang[lang][i].uid, candidate_set.by_lang[lang][i].uid)
    #         print(candidate_set.by_xling_id_get_langs(question_set.by_lang[lang][i].xling_id, languages))
...
def save_data(data, lang, save_dir, split):
    # print("path:", os.path.join(save_dir,"splits", split))
    with open(os.path.join(save_dir,"splits", split, lang + ".json"), "w") as file:
...
def split_lareqa(train_titles, valid_titles, test_titles):
    ...
    # indices = np.arange(len(squad_per_lang["ar"]["data"]))
    ...
    # print("squad_per_lang:", squad_per_lang["de"]["data"][0]["paragraphs"][0]["context"])

    # question_set, _ = utils_lareqa.load_data(squad_per_lang)
    ...
    #         print("--------------------------------------------------------")
    #         for question in question_set.by_xling_id[xling_id]:
    #             print("xling_id:", xling_id, " question.language:", question.language, " question.question:", question.question, " COUNTER:", counter)
    #         print("--------------------------------------------------------")
    #         xling_id = "5727aec03acd2414000de993"
    ...
    for lang in all_languages.split(","):
        # json_sz = len(squad_per_lang[lang]["data"])
        ...
        # squad_per_lang_shuff = squad_per_lang
