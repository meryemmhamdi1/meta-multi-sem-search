...
from multi_meta_ssd.processors.downstream.utils_lareqa import compute_sem_sim
...
class MetaLearner(MetaGeneric):
    ...
    def forward(self, split_name, meta_tasks_batches, ep, batch_step, writer): 
        # Adjust the hyperparameters depending on the split type
        ...
        # Modify the learning rate accordingly
        ...
        for j in pbar:
            ...
            for _ in range(n_up_steps):
                ...
                loss_spt, sent1_encodings_spt, sent2_encodings_spt = spt_outputs
                scores = compute_sem_sim(sent1_encodings_spt, sent2_encodings_spt, meta_tasks_batches[j].spt.scores)
                ...
                del spt_outputs
                del sent1_encodings_spt
                del sent2_encodings_spt
            ...
            loss_qry, sent1_encodings_qry, sent2_encodings_qry = qry_outputs
            scores = compute_sem_sim(sent1_encodings_qry, sent2_encodings_qry, meta_tasks_batches[j].qry.scores)
            ...
            del qry_set
            del qry_outputs
            del sent1_encodings_qry
            del sent2_encodings_qry
            torch.cuda.empty_cache()
            gc.collect()
