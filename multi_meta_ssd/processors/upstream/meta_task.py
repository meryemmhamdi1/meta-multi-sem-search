...
class MetaTask(object):
    ...
    def __init__(self, spt_qst, spt_qst_lang, spt_cand_langs, qry_qst_lang, qry_cand_langs, question_set, candidate_set, meta_learn_split_config, tokenizer, top_results):
        """
        ...
        self.qry_qst_lang = qry_qst_lang ## Assuming the question can be presented in only one language
        self.qry_cand_langs = qry_cand_langs
        ...
        self.neg_trip_margin = 0.2
        self.create_spt_meta_set()
        self.qry = [] # List of MetaSet(s)
        self.qry_features = [] # List of MetaSet(s)
        if self.mode == "similar":
            self.create_qry_meta_sets() # could be one qry set or multiple qry sets depending on how many languages are included
        else:
             self.create_rnd_qry_meta_sets()

    def pick_rnd_qst(self, qry_qst_lang):
        questions_qry_lang = self.question_set.by_lang[qry_qst_lang]
    ...
    def create_qry_meta_sets(self):
        ...
        """
        if self.qry_qst_lang != self.spt_qst_lang:
            for equivalent_question in self.question_set.by_xling_id[self.spt_qst.xling_id]:
                # print("question:", question.language, " self.qry_qst_lang=", self.qry_qst_lang, " self.spt_qst.language:", self.spt_qst.language, " condition:", question.language == self.qry_qst_lang and question.language != self.spt_qst.language)
                if equivalent_question.language == self.qry_qst_lang:
                    positive_candidates = self.candidate_set.by_xling_id_get_langs(equivalent_question.xling_id, self.qry_cand_langs.split(","))
                    ...
                    meta_set = MetaSet(equivalent_question,
                                       positive_candidates=positive_candidates,
                                       negative_candidates=self.get_negative_candidates(equivalent_question, self.qry_cand_langs.split(","), np.mean(d_list)))
                    self.qry.append(meta_set)
                    self.qry_features.append(convert_features(self.meta_learn_split_config, meta_set, self.tokenizer))

        # Find most similar questions in the language of interest
        for xling_id in self.top_results["xling_ids"][self.spt_qst_lang][self.qry_qst_lang][self.spt_qst.xling_id]:
            if xling_id != self.spt_qst.xling_id:
                for similar_question in self.question_set.by_xling_id[xling_id]:
                    if similar_question.language == self.qry_qst_lang:
                        positive_candidates = self.candidate_set.by_xling_id_get_langs(xling_id, self.qry_cand_langs.split(","))
                        ...
                        meta_set = MetaSet(similar_question,
                                           positive_candidates=positive_candidates,
                                           negative_candidates=self.get_negative_candidates(similar_question, self.qry_cand_langs.split(","), np.mean(d_list)))
        ...
        # for question in self.question_set[self.qry_qst_lang]:
        #     question.encoding

    def create_rnd_qry_meta_sets(self):
        random_question = self.pick_rnd_qst(self.qry_qst_lang)
        positive_candidates = self.candidate_set.by_xling_id_get_langs(random_question.xling_id, self.qry_cand_langs.split(","))
        ...
        meta_set = MetaSet(random_question,
                           positive_candidates=positive_candidates,
                           negative_candidates=self.get_negative_candidates(random_question, self.qry_cand_langs.split(","), np.mean(d_list)))
        self.qry.append(meta_set)
        self.qry_features.append(convert_features(self.meta_learn_split_config, meta_set, self.tokenizer))
...
class MetaDataset(object):
    ...
    def __init__(self, n_tasks, lang_pairs, question_set, candidate_set, split, meta_learn_split_config, tokenizer, top_results): # n_train_tasks, n_valid_tasks, n_test_tasks, , train_lang_pairs, valid_lang_pairs, test_lang_pairs):
        ...
        # self.languages = self.question_set.by_lang
        # self.all_embeddings = {lang: [question.encoding for question in self.question_set.by_lang[lang]] \
        #     for lang in self.question_set.by_lang}
        # top_results = {lang1: {lang2: [] for lang2 in self.languages} for lang1 in self.languages}
        # for lang1 in self.languages:
        #     for lang2 in self.languages:
        #         top_k = min(5, len(self.all_embeddings[lang2]))
        #         query_embeddings = self.all_embeddings[lang1]
        #         corpus_embeddings = self.all_embeddings[lang2]
        #         cos_scores = util.cos_sim(query_embeddings, corpus_embeddings)[0]
        #         _top_results = torch.topk(cos_scores, k=top_k)
        #         print(_top_results)
        #         top_results[lang1][lang2] = _top_results
        #         for score, idx in zip(top_results[0], top_results[1]):
        #             print(lang1, lang2, corpus_embeddings[idx], "(Score: {:.4f})".format(score))
        #         exit(0)
